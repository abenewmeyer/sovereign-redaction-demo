...
export default function Page() {
  ...
  const [primaryColor, setPrimaryColor] = useState<string>('#334155') // Bank-grade slate blue fallback
  ...
  // Dynamic branding extraction (logo + primary color via ColorThief)
  useEffect(() => {
    if (!domain) return
    ...
    img.onload = () => {
      try {
        const colorThiefInstance = new (ColorThief as any)()
        const rgb = colorThiefInstance.getColor(img)
        const hex = rgbToHex(rgb[0], rgb[1], rgb[2])
        setPrimaryColor(hex)
      } catch (error) {
        console.error('Color extraction failed (CORS or image issue) — using bank-grade fallback')
        setPrimaryColor('#334155')
      }
    }

    img.onerror = () => {
      console.error('Logo failed to load — using bank-grade fallback')
      setPrimaryColor('#334155')
    }
  }, [domain])

  // Secure processing handler with simulated air-gapped redaction
  const handleProcessSecurely = async () => {
    setIsProcessing(true)
    setSanitizedPayload(null)

    // Simulate local air-gapped redaction pipeline (1.8s delay)
    await new Promise((resolve) => setTimeout(resolve, 1800))

    // Regex-based sanitization (PII redaction)
  ...
  // Landing page (no domain provided)
  if (!domain) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-6">
        <div className="max-w-2xl w-full text-center">
          ...
          <p className="text-xl text-zinc-400 mb-10 max-w-md mx-auto">
            Enter any company domain to instantly brand this bank-grade AI redaction demo.
            <br />
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-2xl">
            <div className="flex flex-col gap-y-2">
              ...
              <div className="flex gap-x-3">
                <input
                  type="text"
                  value={clientName} // Reusing state temporarily for input
                  onChange={(e) => setClientName(e.target.value)} // Temporary reuse
          ...
          <div className="mt-12 text-xs text-zinc-500 flex items-center justify-center gap-x-8">
            ...
            <div>Clearbit logo + dominant color extraction</div>
            <div>Next.js 15 • App Router • Vercel ready</div>
          </div>
        </div>
      </div>
    )
            ...
            {isProcessing ? (
              <div className="space-y-6 w-full">
                ...
                <div className="h-2 bg-zinc-800 rounded-3xl overflow-hidden">
                  <div
                    className="h-2 w-3/4 rounded-3xl animate-[loading_1.5s_infinite_linear]"
              ...
              <div className="text-zinc-400 text-sm leading-relaxed max-w-[220px]">
                Click <span className="font-semibold text-white">PROCESS SECURELY</span> above to trigger local redaction.
                <br /><br />
                Sanitized payload will appear on the right.
              </div>
            )}
          </div>
                ...
                <div className="bg-black/40 border border-zinc-700 rounded-3xl p-6 mb-8 font-mono text-sm">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-zinc-300 leading-snug text-xs">
                        {sanitizedPayload.meetingNotes}
                      </p>
                    </div>
                  </div>
                </div>

                {/* Mock Cloud LLM Summary (only sanitized data) */}
